public class LaberintoBacktracking {
    ...
    public static void main(String[] args) {
        LaberintoBacktracking m = new LaberintoBacktracking(); 						// construimos un objeto de la clase Laberinto por defecto
        m.laberinto[1][1] = 'X';                                                    // introducimos en este caso, la salida (X) en las coordenadas (1,1)
        int x = 8;
        int y = 1;
        m.resuelve(x,y); 															// ahora, introducimos la entrada (S) en las coordenadas (8,1) y llamamos al algoritmo
    ...
    public void resuelve(int x, int y){ // permite introducir unas coordenadas (x, y)
        ...
        if (paso(x,y)) { 								// intentará resolver el laberinto en estas coordenadas
            laberinto[8][1] = 'S'; 						// introduce en las coordenadas (x, y) la entrada
        }
    }
    ...
    private boolean paso(int x, int y)
    {
        ...
        if(x==laberinto.length-1||y==laberinto[x].length-1){
            return false;
        }
